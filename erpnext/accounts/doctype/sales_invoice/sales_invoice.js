...
var field_to_hide_unhide = {
	...
	new_connection_fee: ["customer", "route", "area", "zone", "project",
		"collect_items"],
	deposit: ["customer", "route", "area", "zone", "project", "collect_items"],
	penalty: ["customer", "route", "area", "zone", "collect_items"],
	other: ["customer"],
	all: ["customer", "billing_period", "route", "area", "zone",
		"previous_reading", "current_reading", "consumption",
...
frappe.ui.form.on("Sales Invoice", "collect_items", function (frm) {
	...
	var newrow = cur_frm.grids[0].grid.grid_rows[cur_frm.grids[0].grid.grid_rows.length - 1].doc;
	newrow.item_code = "Deposit(Commercial)"
	cur_frm.refresh_field("items");
	newrow.qty = 1
	cur_frm.refresh_field("items");
	newrow.item_name = "Deposit(Commercial)"
	cur_frm.refresh_field("items");
	newrow.description = "Deposit(Commercial)"
	cur_frm.refresh_field("items");
	newrow.uom = 'Nos'
	cur_frm.refresh_field("items");
	newrow.income_account = "Cost of Goods Sold - UL"
